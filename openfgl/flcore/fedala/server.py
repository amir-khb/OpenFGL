...
class FedALAServer(BaseServer):
    """
    FedALAServer implements the server-side logic for the FedALA algorithm.
    The FedALA algorithm's unique logic (Adaptive Local Aggregation) is
    performed on the client side. The server-side aggregation is
    identical to Federated Averaging (FedAvg).
